...
CREATE TABLE `auction_session` (
  ...
  `is_free_shipping` tinyint(1) DEFAULT '0' COMMENT '是否包邮：0-否，1-是',
  `is_returnable` tinyint(1) DEFAULT '0' COMMENT '是否支持退货：0-否，1-是',
  `min_deposit_amount` decimal(10,0) DEFAULT '100' COMMENT '最小保证金金额（元）',
  `max_bid_amount` decimal(10,0) DEFAULT '1000000' COMMENT '最大出价金额（元）',
  `min_increment_amount` decimal(10,0) DEFAULT '1' COMMENT '最小加价幅度（元）',
...
CREATE TABLE `auction_bid` (
  ...
  `bid_amount_yuan` decimal(10,0) NOT NULL COMMENT '出价金额（元，整数）',
...
CREATE TABLE `auction_result` (
  ...
  `final_price` decimal(10,0) NOT NULL DEFAULT '0' COMMENT '成交价（元）',
  ...
  `commission_fee` decimal(10,0) DEFAULT NULL COMMENT '佣金（元，快照）',
  `deposit_used` decimal(10,0) DEFAULT NULL COMMENT '使用的保证金（元，快照）',
